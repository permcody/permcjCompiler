...
char *versionNumber ="TM version 2.4";
...
/******* const *******/
#define   IADDR_SIZE  2000	/* increase for large programs */
#define   DADDR_SIZE  2000	/* increase for large programs */
#define   NO_REGS 8
#define   PC_REG  7

#define   LINESIZE  200
#define   WORDSIZE  200
#define   DEFAULT_ABORT_LIMIT 5000
...
char *opCodeTab[] = {
    "HALT", "IN", "INB", "OUT", "OUTB", "ADD", "SUB", "MUL", "DIV", "????",
    /* RR opcodes */
    "LD", "ST", "????",		/* RM opcodes */
...
int doCommand(void)
{
    ...
    /***********************************/
	if (atEOL())
	    abortLimit = DEFAULT_ABORT_LIMIT;
	else if (getNum())
	    abortLimit = abs(num);
	else
	    printf("Abort limit?\n");
    ...
    /***********************************/
	    ...
	    cnt = 0;
	    for (i = 0; i<IADDR_SIZE; i++) if (dMemTag[i]==USED) cnt++;
    ...
    /***********************************/
	   ...
	   while ((dloc >= 0) && (dloc<DADDR_SIZE) && (printcnt>0)) {
	       ...
	       if (dMemTag[dloc]==USED) printf("    used\n");
	       else printf("\n");
	       if (down) dloc--;
	       else dloc++;
	       printcnt--;
	...
	if (cmd == 'g') {
	    stepcnt = 0;
	    while ((stepResult == srOKAY) && (stepcnt<abortLimit)) {
		iloc = reg[PC_REG];
		if (traceflag)
